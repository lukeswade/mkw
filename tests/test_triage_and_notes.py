...
def _long_article(title: str) -> str:
    """A fixture page big enough to cross the screening threshold."""
    body = "".join(f"<p>Padding paragraph {i} about {title.lower()} with "
                   f"plenty of words to inflate the length beyond the "
                   f"screening minimum, section {i}.</p>" for i in range(40))
    return (f"<html><head><title>{title}</title></head><body><main><article>"
            f"<h1>{title}</h1>{body}</article></main></body></html>")


@respx.mock
async def test_screen_skips_junk_before_the_full_notes_call(data_dir):
    cfg = make_cfg(data_dir)
    respx.get(f"{SX}/search").mock(return_value=httpx.Response(200, json=sx_payload(
        [sx_result("https://junk.example.com/page", "Junk Page")])))
    respx.get("https://junk.example.com/page").mock(
        return_value=httpx.Response(200, html=_long_article("Junk Page")))

    s = script([{"state_md": "s", "saturated": True, "next_queries": []}])
    s["screen"] = [{"relevance": 1}]
    repo, llm, orch, run_id = _run(make_cfg(data_dir), s)
    await orch.execute_now(run_id)

    assert llm.calls["screen"] == 1
    assert llm.calls["notes"] == 0            # the full read never happened
    assert repo.findings_for_run(run_id) == []
    cfg = orch.cfg_loader()
    events = (cfg.research_dir / run_id / "events.jsonl").read_text()
    assert "screened out (1/10 on first look)" in events


@respx.mock
async def test_screen_pass_reaches_full_notes(data_dir):
    cfg = make_cfg(data_dir)
    respx.get(f"{SX}/search").mock(return_value=httpx.Response(200, json=sx_payload(
        [sx_result("https://good.example.com/page", "Good Page")])))
    respx.get("https://good.example.com/page").mock(
        return_value=httpx.Response(200, html=_long_article("Good Page")))

    s = script([{"state_md": "s", "saturated": True, "next_queries": []}])
    s["screen"] = [{"relevance": 7}]
    repo, llm, orch, run_id = _run(cfg, s)
    await orch.execute_now(run_id)
    assert llm.calls["screen"] == 1
    assert llm.calls["notes"] == 1
    assert len(repo.findings_for_run(run_id)) == 1


@respx.mock
async def test_small_documents_bypass_the_screen(data_dir):
    """Small pages go straight to notes — cheap anyway, and index stubs must
    reach the notes stage where their child links get chased."""
    cfg = make_cfg(data_dir)
    respx.get(f"{SX}/search").mock(return_value=httpx.Response(200, json=sx_payload(
    ...
    s = script([{"state_md": "s", "saturated": True, "next_queries": []}])
    # no "screen" entry at all — a screen call would raise in FakeLLM
    repo, llm, orch, run_id = _run(cfg, s)
    await orch.execute_now(run_id)
    assert llm.calls["screen"] == 0
    assert len(repo.findings_for_run(run_id)) == 1


@respx.mock
async def test_screen_failure_degrades_to_full_read(data_dir):
    cfg = make_cfg(data_dir)
    respx.get(f"{SX}/search").mock(return_value=httpx.Response(200, json=sx_payload(
        [sx_result("https://good.example.com/page", "Good Page")])))
    respx.get("https://good.example.com/page").mock(
    ...
    s = script([{"state_md": "s", "saturated": True, "next_queries": []}])
    s["screen"] = [LLMJsonError("garbled")]
    repo, llm, orch, run_id = _run(cfg, s)
    await orch.execute_now(run_id)
    assert llm.calls["notes"] == 1            # fell through to the full read
    assert len(repo.findings_for_run(run_id)) == 1
